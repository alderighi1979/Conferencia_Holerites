...
import { inssAPI } from '../../services/api';
...
function TabINSS() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = {
        ...
        valor_deduzir: parseFloat(formData.valor_deduzir) || 0,
      };

      if (editando) {
        await inssAPI.update(editando.id, data);
      } else {
        await inssAPI.create(data);
      }
  ...
  return (
    <div>
      <h2 className="text-xl font-semibold text-gray-900 mb-4">Tabela INSS</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <input
              type="number"
              step="0.01"
              value={formData.valor_deduzir}
              onChange={(e) => setFormData({ ...formData, valor_deduzir: e.target.value })}
              className="w-full px-3 py-2 border border-gray-300 rounded-md"
            />
          </div>
        </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {faixas.map((faixa) => (
                <tr key={faixa.id}>
                  ...
                  <td className="px-4 py-3 whitespace-nowrap text-sm">
                    R$ {faixa.valor_deduzir.toFixed(2)}
                  </td>
                  <td className="px-4 py-3 whitespace-nowrap text-sm">
                    <button
